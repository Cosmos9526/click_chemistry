...
# --- Main parameters (BCN-focused, cleavable) ---
unique_smiles: Dict[str, str] = {
    ...
    # Click/BCN chloroformate for carbamate (from example)
    'BCN-chloro': 'ClC(=O)OCC1[C@H]2CCC#CCC[C@@H]12',
...
# Component categories (BCN-only for click)
...
reaction_smarts: List[str] = [
    '[O:1][C:2](=[O:3])[Cl].[NH2:4]>>[O:1][C:2](=[O:3])[NH:4].[Cl]',  # Carbamate (BCN-chloro + NH2)
    '[C:1](=[O:2])[OH].[NH2:3]>>[C:1](=[O:2])[NH:3]',  # Amide (COOH + NH2)
...
def generate_random_pattern() -> Tuple[List[str], str]:
    """Generate random pattern: BCN + spacer + cleavable + opt spacer + self-immolative"""
    cleavage_types = ['enzyme', 'disulfide', 'pH', 'glycosidase']
    cleavage_type = random.choice(cleavage_types)
    
    pattern = []
    
    # BCN-chloro + spacer (carbamate)
    pattern.append('BCN-chloro')
...
def generate_linker(components: List[str]) -> str:
    """Connect components using reactions (carbamate first for BCN)"""
    try:
...
print(f"Starting generation of {target} unique BCN linkers...")
...
# Save
with open('bcn_cleavable_linkers.json', 'w', encoding='utf-8') as f:
    json.dump(successful, f, ensure_ascii=False, indent=2)

print("Saved to bcn_cleavable_linkers.json!")

# Test with example pattern
test_pattern = ['BCN-chloro', 'PEG3', 'Val-Cit', 'PAB-OH']
test_smiles = generate_linker(test_pattern)
print("\nExample SMILES (BCN-PEG3-ValCit-PABOH):", test_smiles)
